...
def write_plasma_tie(rows) -> None:
    tie = [[1 if pixel == "#" else 0 for pixel in row]
           for row in PLASMA_TIE_PATTERN]
    assert len(tie) == PLASMA_TIE_SIZE[1]
    assert all(len(row) == PLASMA_TIE_SIZE[0] for row in tie)
    frames = [rotate_to_sprite(tie, frame * math.pi / PLASMA_TIE_FRAMES)
    ...
    for frame, data in enumerate(frames):
        lines.append(f"; rotation frame {frame}")
    ...
    counts = ", ".join(str(sum(byte.bit_count() for byte in frame))
                       for frame in frames)
    print(f"Wrote {PLASMA_OUTPUT_PATH.relative_to(ROOT)} "
          f"({PLASMA_TIE_FRAMES} frames, pixels={counts})")
