#!/usr/bin/env python3
"""Convert the Studio3S presents master into a packed four-color VZ200 frame."""
...
HEIGHT = 56
ROW_BYTES = WIDTH // 4
FILTER = "crop=1528:668:0:180,scale=128:56:flags=neighbor,format=rgb24"


def parse_ppm(path: Path) -> bytes:
    magic, dimensions, maximum, pixels = path.read_bytes().split(b"\n", 3)
    if magic != b"P6" or dimensions != b"128 56" or maximum != b"255":
...
def main() -> None:
    ...
    if ffmpeg is None:
        raise SystemExit("ffmpeg is required to generate the VZ200 presents frame")

    with tempfile.TemporaryDirectory() as directory:
        image_path = Path(directory) / "presents.ppm"
        subprocess.run(
            [
                ffmpeg,
                "-y",
    ...
    rows = [
        "; Generated from ports/vz200/assets/presents-master.png. Do not edit.",
        f"PRESENTS_FRAME_ROW_BYTES EQU {ROW_BYTES}",
        f"PRESENTS_FRAME_HEIGHT EQU {HEIGHT}",
        f"PRESENTS_FRAME_BYTES EQU {ROW_BYTES * HEIGHT}",
        "PresentsFrame:",
    ]
    for row in range(HEIGHT):
        start = row * ROW_BYTES
