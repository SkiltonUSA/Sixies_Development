...
PRG_PATH="$BUILD_DIR/dhaka.prg"
D64_PATH="$BUILD_DIR/dhaka.d64"
...
<html lang="en">
  ...
  <body>
    <main>
      <section class="card">
        <h1>Nobounds Plasma Menu</h1>
        <p>The local build is ready and VICE should be opening on this Mac.</p>
        <p>Artifacts: <a href="/dhaka.prg">dhaka.prg</a>, <a href="/dhaka.d64">dhaka.d64</a></p>
...
cd "$BUILD_DIR"
echo "Launching VICE with $D64_PATH"
"$VICE_BIN" +logtofile -logfile "$VICE_STATE_DIR/vice.log" \
  -8 "$D64_PATH" -autostart-warp -autostart "$D64_PATH" >/dev/null 2>&1 &
