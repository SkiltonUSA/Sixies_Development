...
`make setup-nes` verifies the NES toolchain, creates the `ports/nes/`
workspace layout, and writes absolute path exports to the gitignored
`.context/nes-toolchain.env`.
...
- `ports/nes/` is the Nintendo NES port workspace. Keep rules portable and
  deterministic there before adding PPU, APU, or mapper-specific code.
...
uses directions and fire. The `.` key randomly fills the board and is a
development-only endgame shortcut.
...
For the Game Boy work, follow `docs/porting-gameboy.md`. For the NES work,
follow `docs/porting-nes.md`. In both cases, the first milestone is a
host-testable portable rules engine and the second is a simple playable board.
