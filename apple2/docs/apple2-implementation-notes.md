...
- `assets/game_grid_dhgr_mono_master.a2fm` and its reference PNG preserve the supplied full-screen 560-pixel monochrome layout. `scripts/import_a2fm_grid.py` verifies every source pixel, then replaces only the old reminder panel interior with a static `CUR` label before packing `GRID.A2FM`.
...
- The importer replaces the noisy source `SCORE`, `TOTAL`, `NEW GAME`, and `SETTINGS` lettering with pre-rendered opaque monochrome glyphs while retaining the surrounding panels, buttons, and settings icon. It also clears the baked five-digit value. Runtime digits are XOR-blitted into that well so every grid reload can restore the current total. Only the first two verified-black wells in the right panel display the current piece; the unused next-piece area remains blank.
