...
- `assets/title_dhgr_mono_master.a2fm` is the supplied 16 KB b2d monochrome DHGR title. The source remains unchanged and is verified pixel-for-pixel against `assets/title_dhgr_mono_reference.png` before `scripts/generate_title.py` adds the `PRESS SPACE` prompt in the lower-center clear area.
- The A2FM layout stores the 8 KB auxiliary page first and the 8 KB main page second. The runtime packages the prompt-enhanced output as one 16 KB `TITLE.A2FM` file and streams both banks through one ProDOS open, sharing the grid's loader.
...
- The attract loop polls the keyboard once per vertical blank. `Space`, `Return`, or `N` starts from any page; if no start key arrives, the initial title advances to instructions after ten seconds and the live high-score table, presentation, title, and instructions then rotate every five seconds. The table reloads `HISCORE` on each pass so disk saves appear immediately. Pending repeats are drained only after startup exits, before the normal grid assets load.
- `scripts/generate_high_score_screen.py` converts the supplied 64x56 dice pair to stable monochrome DHGR, doubles its signal width for the Apple II pixel aspect ratio, and places it in the right panel of `HISCORES.RLE`. Fixed ranks, borders, labels, and prompts are pre-rendered; the live initials, right-aligned scores, and optional rank marker are drawn after each load.
- The live table uses aligned 14-signal character cells. `HSFONT` stores seven auxiliary bytes followed by seven main bytes for each glyph, totaling 560 disk-backed bytes. A compact assembly writer copies one byte per bank per scanline and reuses the dice buffer, row-address workspace, and DHGR page, so the artwork adds no dedicated screen or font BSS allocation. Gameplay reloads `DICE.BLITS` before drawing the board.
...
- The centered DHGR table reuses the 1 KB transfer buffer, adding no dedicated high-score array to BSS. The emulator launcher preserves the writable `HISCORE` file while replacing the rest of its working disk with the latest release build.
...
- Merges consuming face value 5 or 6 add four diagonal arms during those same five ripple steps. The diagonal walker stops before XOR-inverting the destination because the horizontal/vertical cross already owns that cell, so overlapping paths cannot cancel the merged-die flash and the enhancement adds no animation frames.
