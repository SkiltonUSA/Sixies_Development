...
- `build/star-wars-40s.sid` -- PAL PSID tune, stopped after exactly 40 seconds
- `build/star-wars-40s.prg` -- assembled C64 music driver used by the PSID
...
The 40-second SID is a hand-arranged three-voice version based on the supplied
MP3 reference, generated by `scripts/generate_starwars_sid_frames.py`. It does
not use the vendored NoBounds music. Its PSID init/play addresses are `$1000`
and `$1080`; playback ends after 2,000 PAL frames. The original mid-1980s C64
soundtrack arrangement uses a focused lead line, eight-step chord arpeggios,
pulse-width modulation, note bends, lead vibrato, hard-restarted envelopes,
bass-voice noise percussion, octave punches, and a resonant low-pass sweep.
